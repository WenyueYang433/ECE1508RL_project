...
def prepare_evaluation_data(
    ...
    key_to_movieid = dict(
        zip(mf_sorted["movie_key"].to_numpy(), mf_sorted["movieId"].to_numpy())
    )
    ...
    user_state = build_user_state_vectors(
        train_df, item_matrix, history_window, is_gru=is_gru
    )
...
def _intra_list_diversity(
    rec_items: List[int], item_features: Dict[int, np.ndarray]
) -> float:
    if len(rec_items) < 2:
        return 0.0
...
def eval_prcp(
    ...
    # user actual rating
    rating_lookup = dict(
        zip(zip(test_df_id["userId"], test_df_id["movieId"]), test_df_id["rating"])
    )
    ...
    for u in users:
        ...
        for item in rec_items:
            if item in test_items:
                hits.append(1)
                rating_val = rating_lookup.get((u, item), 3.0)
        ...
        metrics["map"].append(
            _average_precision(hits, min(len(test_items), len(rec_items), N))
        )
    ...
    results = {
        ...
        "HitRate": (
            hit_users / len(metrics["precision"]) if metrics["precision"] else 0.0
        ),
        "Coverage@Catalog": len(set(all_rec_items)) / float(n_items_total),
        "Coverage@User": np.mean(
            metrics["coverage_user"]
        ),  #  this metrics is  no longer in use, it's  mathematically forced to be 0.01.
...
def analyze_q_distribution(agent, data, hp, project_root):
    ...
    with torch.no_grad():
        for u in test_users:
            state = (
                torch.tensor(data["user_state"][u], dtype=torch.float32)
                .unsqueeze(0)
                .to(device)
            )
    ...
    save_path = (
        ...
        / "reports"
        / "figures"
        / f"q_dist_penalty_{hp.popularity_penalty}.png"
    )
...
def build_user_state_vectors(
    train_df, item_matrix: np.ndarray, history_window: int, is_gru: bool = False
) -> dict[int, np.ndarray]:
...
def make_dqn_recommender(
    dqn_model: DQN, user_state, seen_train, movie_ids_by_key, n_actions, device
):
    dqn_model.eval()
...
def _print_metrics_block(
    title: str, metrics: dict[str, float | dict], indent: int = 0
) -> None:
    prefix = " " * indent
...
def run_evaluation(
    ...
    if model_path is None:
        model_path = PROJECT_ROOT / hp.model_base
    ...
    dqn_rec = make_dqn_recommender(
        dqn,
        data["user_state"],
        data["seen_train"],
        data["movie_ids_by_key"],
        data["n_actions"],
        device,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Offline evaluation using Hyperparameters."
    )
    parser.add_argument(
        "--model-type",
        type=str,
        choices=["base", "finetuned", "custom"],
        default="base",
        help="Which config path to use.",
    )
    parser.add_argument("--custom-path", type=str, help="Path if model-type is custom.")
    parser.add_argument(
        "--no-plots", default=False, action="store_true", help="Disable plots."
    )
