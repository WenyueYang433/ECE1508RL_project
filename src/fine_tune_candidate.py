...
def fine_tune(model_path_str: str):
    ...
    print(
        f"Arch: {hp.model_arch} | Steps={hp.ft_n_steps}, LR={hp.ft_lr}, Margin={hp.margin}"
    )
    ...
    optimizer = optim.Adam(
        dqn.parameters(), lr=hp.ft_lr, weight_decay=hp.ft_weight_decay
    )
    ...
    print(
        f"Base Precision@10: {before_metrics['Precision@10']:.4f} | Base NDCG@10: {before_metrics['NDCG@10']:.4f}"
    )
    ...
    for step in range(1, hp.ft_n_steps + 1):
        ...
        for u in batch_users:
            ...
            pool = (
                candidates_by_user.get(u, candidates_by_user.get("global", []))
                if hp.use_candidates
                else []
            )
            ...
            n_cand = (
                int(round(hp.neg_per_pos * hp.frac_candidate))
                if hp.use_candidates
                else 0
            )
        ...
        if step % 200 == 0:
            ...
            rec_func = make_dqn_recommender(
                dqn, user_state, seen_train, movie_ids_by_key, n_actions, device
            )
    ...
    final_rec = make_dqn_recommender(
        dqn, user_state, seen_train, movie_ids_by_key, n_actions, device
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the .pt model file to fine-tune",
    )
